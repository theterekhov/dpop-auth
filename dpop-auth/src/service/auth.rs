//! Registration, login, refresh-token rotation (RTR), and logout.
...
use crate::{
    ServiceError,
    crypto::{
        entropy::{identifier_hash, new_refresh_secret},
        hash_token, password,
    },
    store::{
        models::{CreateRefreshTokenParams, UserRow},
        repo::identifiers as repo_identifiers,
        repo::login_attempts as repo_attempts,
        repo::refresh_tokens as repo_tokens,
        repo::users as repo_users,
    },
};

use super::{AuthService, ReplacementTokens};
...
pub struct RegisterParams<'a> {
    ...
    /// Display or full name of the user.
    pub name: &'a str,
...
impl AuthService {
    ...
    async fn issue_session(
        ...
        repo_tokens::create_refresh_token(
            conn,
    ...
    async fn authenticate_user(
        ...
        let opt_user = repo_identifiers::find_user_by_identifier(conn, kind, value).await?;
        ...
        match opt_user {
            ...
            Some(_) => {
                repo_attempts::record_login_attempt(
                    conn,
                    kind,
                    value,
    ...
    /// # Errors
    ///
    /// Returns [`ServiceError::RegistrationDisabled`] if self-registration is disabled.
    ...
    pub async fn register(&self, params: RegisterParams<'_>) -> Result<TokenPair, ServiceError> {
        ...
        let user =
            repo_users::create_user(&mut tx, params.name, &password_hash, Some(Utc::now())).await?;

        if let Err(e) = repo_identifiers::create_identifier(
            &mut tx,
            user.id,
            params.kind,
            params.value,
        ...
        repo_users::update_last_login_at(&mut tx, user.id).await?;
    ...
    pub async fn login(
        ...
        repo_users::update_last_login_at(&mut conn, user.id)
            .await
            .ok();
        repo_attempts::record_login_attempt(
            &mut conn,
    ...
    pub async fn refresh(
        ...
        let row = repo_tokens::revoke_refresh_token_if_active(&mut tx, &token_hash).await?;

        let Some(row) = row else {
            // Already revoked or unknown: distinguish benign grace retry from malicious reuse.
            let existing = repo_tokens::find_refresh_token_by_hash(&mut tx, &token_hash).await?;
            ...
            match existing {
                Some(existing) => {
                    ...
                    repo_tokens::revoke_refresh_token_family(&mut tx, existing.fam).await?;
        ...
        let user = repo_users::find_user_by_id(&mut tx, row.user_id)
            .await?
    ...
    pub async fn logout(&self, refresh_token: &str) -> Result<(), ServiceError> {
        ...
        let mut conn = self.pool.acquire().await?;
        repo_tokens::revoke_refresh_token_by_hash(&mut conn, &token_hash).await?;
...
mod tests {
    ...
    use crate::{DpopConfig, TokenSigner};
    ...
    fn build_service(
        ...
        let signer = TokenSigner::symmetric(b"test-secret-key-must-be-at-least-32-bytes").unwrap();
    ...
    async fn refresh_expiry(pool: &PgPool, secret: &str) -> DateTime<Utc> {
        ...
        repo_tokens::find_refresh_token_by_hash(&mut conn, &hash_token(secret.as_bytes()))
            .await
            .unwrap()
            .unwrap()
            .expires_at
    ...
    async fn refresh_rotates_token(pool: PgPool) {
        ...
        let old_row = repo_tokens::find_refresh_token_by_hash(
            &mut conn,
            &hash_token(old.refresh_token.as_bytes()),
        )
        .await
        .unwrap()
        .unwrap();
        ...
        let new_row = repo_tokens::find_refresh_token_by_hash(
            &mut conn,
            &hash_token(new.refresh_token.as_bytes()),
        )
        .await
        .unwrap()
        .unwrap();
