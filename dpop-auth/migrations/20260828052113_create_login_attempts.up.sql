...
-- TABLE
CREATE TABLE IF NOT EXISTS dpop_login_attempts (
...
-- INDEXES
CREATE INDEX IF NOT EXISTS idx_dpop_login_ident
	ON dpop_login_attempts (identifier_kind, LOWER(identifier_value), created_at DESC);

CREATE INDEX IF NOT EXISTS idx_dpop_login_ip
	ON dpop_login_attempts (ip_address, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_dpop_login_created
